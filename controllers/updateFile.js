...
module.exports.updateFile = function updateFile(req, res, next) {
  var title = req.swagger.params["title"].value;
  var file = req.swagger.params["file"].value;
  var email = req.auth.email;
  var id = req.auth.sub;
  File.updateFile(title, file)
    .then(async function(response) {
      ...
      console.log(cid); //logs the CID AND PATH OF THE FILE + SIZE

      web3.eth.getTransactionCount(
        config.api_contract.owner.public_key,
        (err, txCount) => {
          var ABI = APIContract.methods
            .updateHashOfFile(title, cid.toString(), id, email) //updates the hash of the file stored in the smart contract
            .encodeABI();
          ...
          const transaction = web3.eth
            ...
            .on("receipt", function(receipt) {
              if (receipt["status"] == true) {
                utils.writeResponse(
                  res,
                  {
                    status: "The file was updated with success",
                    title: title,
                    cid: cid.toString(),
                  },
                  200
            ...
            .on("error", function(error) {
              utils.writeResponse(res, {
                status:
                  "Something went wrong trying to update your file, make sure that the file exists !  ",
              });
            });
        }
      );
